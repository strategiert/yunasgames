...
const PetGame = () => {
  // Game states
  const [screen, setScreen] = useState('start'); // start, choosePet, main, collar, shop, house
  ...
  const [mood, setMood] = useState('happy'); // happy, sad, sleeping, eating, playing
  ...
  }, [hunger, sleep, fun, toilet, isSleeping]);
  ...
  const renderPet = () => {
    ...
    return (
      <div className="relative flex flex-col items-center">
        {/* Pet */}
        <div className="text-8xl mb-2 transition-transform duration-300" 
             style={{ transform: mood === 'playing' ? 'scale(1.1) rotate(5deg)' : 'scale(1)' }}>
          {getPetEmoji()}
        </div>
        {/* Collar */}
        <div 
          className="absolute bottom-16 w-16 h-3 rounded-full flex items-center justify-center"
          style={{ backgroundColor: collarColor }}
        >
          {hasBell && <span className="text-xs">🔔</span>}
        </div>
        {/* Mood indicator */}
        <div className="text-2xl">{moodFaces[mood]}</div>
        {/* Name */}
        <div className="text-lg font-bold mt-1">{petName}</div>
      </div>
    );
  };
        ...
        {petType && (
          <div className="w-full max-w-xs">
            <label className="block text-purple-700 font-medium mb-2">
              Wie soll dein {petType === 'cat' ? 'Kätzchen' : petType === 'dog' ? 'Hündchen' : 'Häschen'} heißen?
            </label>
            <input
              type="text"
              value={nameInput}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-amber-100 to-orange-100 p-4">
        ...
        <div className="bg-amber-50 rounded-2xl p-6 shadow-lg max-w-sm mx-auto min-h-64 relative border-4 border-amber-300">
          {/* Room decoration */}
          <div className="absolute top-2 left-2">{furniture.poster && '🖼️'}</div>
          <div className="absolute top-2 right-2">{furniture.plant && '🪴'}</div>
          <div className="absolute bottom-2 left-2">{furniture.bed && '🛏️'}</div>
          <div className="absolute bottom-2 right-2">{furniture.toilet && '🚽'}</div>
          <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2">
            {furniture.carpet && <div className="text-3xl">🟫</div>}
          </div>
          <div className="absolute top-1/2 left-4">{furniture.bowl && '🥣'}</div>
  ...
  // MAIN GAME SCREEN
  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-200 to-green-300 p-4">
      {/* Header */}
      <div className="flex justify-between items-center mb-4">
        <div className="bg-yellow-400 px-4 py-2 rounded-full font-bold shadow">
          💰 {coins}
        </div>
        <div className="flex gap-2">
          <button onClick={() => setScreen('collar')} className="text-2xl bg-white/50 p-2 rounded-full">👔</button>
          <button onClick={() => setScreen('shop')} className="text-2xl bg-white/50 p-2 rounded-full">🛒</button>
          <button onClick={() => setScreen('house')} className="text-2xl bg-white/50 p-2 rounded-full">🏠</button>
        </div>
      </div>

      {/* Status bars */}
      <div className="bg-white/80 rounded-2xl p-4 mb-4 shadow-lg">
        <StatusBar label="Hunger" value={hunger} color="#22C55E" icon="🍖" />
        <StatusBar label="Schlaf" value={sleep} color="#3B82F6" icon="😴" />
        <StatusBar label="Spaß" value={fun} color="#F59E0B" icon="⭐" />
        <StatusBar label="Toilette" value={toilet} color="#8B5CF6" icon="🚽" />
      </div>

      {/* Pet display area */}
      <div className="bg-gradient-to-b from-green-200 to-green-400 rounded-2xl p-6 mb-4 shadow-lg relative overflow-hidden">
        {/* Background elements */}
        <div className="absolute top-2 right-4 text-4xl opacity-80">☀️</div>
        <div className="absolute bottom-0 left-4 text-2xl">🌸</div>
        <div className="absolute bottom-0 right-8 text-xl">🌼</div>
        
        {/* Sleeping overlay */}
        {isSleeping && (
          <div className="absolute inset-0 bg-indigo-900/50 rounded-2xl flex items-center justify-center">
        ...
        {needsClean && (
          <button
            onClick={clean}
            className="absolute bottom-2 right-2 bg-yellow-400 px-3 py-1 rounded-full text-sm font-bold animate-pulse"
          >
            🧹 Sauber machen! (+2💰)
          </button>
        )}
      ...
      <div className="grid grid-cols-3 gap-3">
        <button
          ...
          className="bg-green-500 hover:bg-green-600 disabled:bg-gray-400 text-white p-4 rounded-2xl shadow-lg flex flex-col items-center"
        >
          <span className="text-2xl">🍖</span>
          <span className="text-xs mt-1">Füttern</span>
        ...
        <button
          ...
          className="bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 text-white p-4 rounded-2xl shadow-lg flex flex-col items-center"
        >
          <span className="text-2xl">☕</span>
          <span className="text-xs mt-1">Kakao</span>
        ...
        <button
          onClick={goSleep}
          disabled={isSleeping}
          className="bg-indigo-500 hover:bg-indigo-600 disabled:bg-gray-400 text-white p-4 rounded-2xl shadow-lg flex flex-col items-center"
        >
          <span className="text-2xl">🛏️</span>
          <span className="text-xs mt-1">Schlafen</span>
        </button>

        <button
          onClick={useToilet}
          disabled={isSleeping || needsClean}
          className="bg-purple-500 hover:bg-purple-600 disabled:bg-gray-400 text-white p-4 rounded-2xl shadow-lg flex flex-col items-center"
        >
          <span className="text-2xl">🚽</span>
          <span className="text-xs mt-1">Toilette</span>
        </button>

        <button
          onClick={play}
          disabled={isSleeping}
          className="bg-orange-500 hover:bg-orange-600 disabled:bg-gray-400 text-white p-4 rounded-2xl shadow-lg flex flex-col items-center col-span-2"
        >
          <span className="text-2xl">⚽</span>
          <span className="text-xs mt-1">Spielen (+3💰)</span>
        </button>
      </div>
